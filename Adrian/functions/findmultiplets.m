...
% If the ripples are closer to 200ms we consider the ripples to be multiplets.
...
% and ripple 3 of a triplet is in Mx_multiplets.triplets.m_3
    ...
    for l=1:length(Mx)
         hfo_sequence=ConsecutiveOnes(diff(Mx{l})<=0.200);
...
%                   eval([  'clear' ' ' 'Mx_' multiplets{ll} '_' num2str(cont)])
            ...
            for i=2:length(multiplets)
                multiplets_stamps=[multiplets_stamps; reshape(M_multiplets.(multiplets{i}){:},[],1)];
            end
