...
function hmm_all_results=fun_HMM_decoding(spikes,hmm_bestfit,HmmParam,win_train,spikes_peak)
...
%         end
        ...
        for st=1:NumStates
            for unit=1:gnunits
                ...
                % turn spikes into yt
                if strcmp(DISTR,'poisson')
                    spks=hmm.Spikes2Seq(spikes_peak(trial,unit),win,BinSize,DISTR); % get spike count
                    A=(pstates_rate(st,:)*spks')/(pstates_rate(st,:)*v1);
                    lambda(st,unit)=(A/BinSize);
                else
                    temp_spikes=spikes_peak(trial,unit).spk;
