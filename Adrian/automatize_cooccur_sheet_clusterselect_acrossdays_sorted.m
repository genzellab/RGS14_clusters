...
for r = 1:length(rat_dirs)
    ...
    % Get list of relevant files in this rat folder
    pattern = sprintf('Subsampled2000HPC_Cluster_%s_CO_*.mat', cluster);
    files = dir(pattern);
    ...
    for k = 1:length(files)
        ...
        spindle_var = sprintf('CO_count_spindle_%s', lower(cluster));
        ripple_var  = sprintf('CO_count_ripple_%s', lower(cluster));
