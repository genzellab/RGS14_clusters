% MATLAB script to extract del_spinwrip counts for C1 and C2 side by side

all_data_map = containers.Map(); % key: "ratID_studyDay", value: struct with fields C1, C2

% Get list of rat folders (assumed to be numeric)
rat_dirs = dir();
...
for r = 1:length(rat_dirs)
    ...
    files = dir(fullfile(rat_path, 'Subsampled2000HPC_Cluster_C*_del_spinwrip_*.mat'));
...
rows = zeros(0, 4);  % [RatID, StudyDay, Spinwrip_C1, Spinwrip_C2]
...
fprintf('✅ Saved del_spinwrip data with C1 and C2 columns to %s\n', outfile);
